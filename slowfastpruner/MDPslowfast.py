...
class MDPPruner:
    def __init__(self, model, sparsity=0.5, device='cuda:0'):
        ...
        self.activations = {}
        self.hooks = []
        self.layer_avg_scores = {}  # 用于统计打印

    def _get_activation_hook(self, name):
        def hook(module, input, output):
            # 获取输出激活值的 L2 Norm [B, C, T, H, W]
            data = output.detach().float()
            # 针对卷积层：对 B, T, H, W 维度求 Norm，保留 C 维度
            reduce_dims = [0, 2, 3, 4] 
            current_norm = torch.norm(data, p=2, dim=reduce_dims)

            if name not in self.activations:
                self.activations[name] = current_norm
            else:
                # 移动平均更新 (EMA)
                self.activations[name] = 0.9 * self.activations[name] + 0.1 * current_norm.to(self.device)
        return hook
    ...
    def run_calibration(self, loader, device):
        """保持接口一致，进行前向传播以收集激活信息"""
        self.register_hooks()
        self.model.eval()
        print(">>> MDP Pruner: 正在收集权重-激活响应特征...")
        with torch.no_grad():
    ...
    def prune(self, min_keep_ratio=0.1):
        """
        MDP 核心逻辑：Score = (Weight_Norm * Activation_Norm) / Cost
        """
        all_candidates = []
        valid_layer_names = [n for n, m in self.model.named_modules() if hasattr(m, 'weight') and n in self.activations]
        
        # 1. 收集全局所有通道的 MDP 评分
        for name in valid_layer_names:
            m = dict(self.model.named_modules())[name]
            act_norm = self.activations[name] # [C]
            
            # 计算权重响应：每个输出通道的权重的 L1 Norm
            ...
            # MDP 评分公式
            scores = (w_norm * act_norm) / cost
            
            # 记录平均得分用于统计
            self.layer_avg_scores[name] = scores.mean().item()
            
            # 每一层的最小保留数
            max_prunable = m.out_channels - max(1, int(m.out_channels * min_keep_ratio))
        ...
        layer_pruned_count = {name: 0 for name in valid_layer_names}
        ...
        for cand in all_candidates:
            ...
            ln = cand['layer']
            if layer_pruned_count[ln] < cand['max_prunable']:
                registry[ln].add(cand['idx'])
                current_red += cand['cost']
                layer_pruned_count[ln] += 1

        # 4. 生成 interaction_mask (保持 Bottleneck 逻辑兼容)
        for name in valid_layer_names:
            m = dict(self.model.named_modules())[name]
        ...
        print(f">>> MDP Pruning 完成。")
        return {'actual_sparsity': current_red / total_p}

    def get_stats(self):
        """保持接口不变，打印层级统计数据"""
        print("\n" + "="*85)
        print(f"{'Layer Name':<40} | {'MDP Score':<10} | {'Kept/Total':<15} | {'Ratio'}")
        print("-" * 85)
