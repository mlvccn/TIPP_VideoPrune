...
class PlugPruner:
    def __init__(self, model, sparsity=0.5, device='cuda:0'):
        ...
        self.original_total_params = sum(p.numel() for p in model.parameters())

    def _get_activation_hook(self, name):
        def hook(module, input, output):
            z = output.detach()
            # 针对通道剪枝优化：将 [B, C, T, H, W] 池化为 [B, C]
            # 这样 HSIC 计算的就是通道间的相关性，而不是时空维度的相关性
            if len(z.shape) == 5: 
                z_final = torch.mean(z, dim=(2, 3, 4)) 
            else: 
                z_final = z.reshape(z.shape[0], -1)
        ...
        with torch.no_grad():
            for i, batch in enumerate(tqdm.tqdm(loader, desc="Calibration")):
                ...
                if isinstance(vids, list):
                    vids = [v.to(self.device) for v in vids]
                else:
    ...
    def prune(self, min_keep_ratio=0.1):
        ...
        for idx, name in enumerate(valid_layer_names):
            ...
            self.layer_avg_scores[name] = scores[idx].item()
            
            out_c = m.out_channels if hasattr(m, 'out_channels') else m.out_features
            w_norm = torch.norm(m.weight.data.view(out_c, -1), p=1, dim=1)
            norm_scores = (w_norm - w_norm.min()) / (w_norm.max() - w_norm.min() + 1e-8)
            combined_scores = norm_scores * (0.5 + scores[idx])
            
            cost = (m.in_channels * math.prod(m.kernel_size)) if isinstance(m, nn.Conv3d) else m.in_features
            max_p = out_c - max(1, int(out_c * min_keep_ratio))
            layer_info[name] = {'total': out_c, 'pruned': 0, 'max_prunable': max_p}

            for i, s in enumerate(combined_scores):
                prio = s.item() / (cost + 1e-9)
                heapq.heappush(pq, [prio, name, i, s.item(), cost, 0])

        # 4. 执行贪婪剪枝
        target_red = self.original_total_params * self.sparsity
        current_red = 0
        ...
        with tqdm.tqdm(total=int(target_red), desc="IT-Pruning") as pbar:
            while current_red < target_red and pq:
                ...
                if last_cnt != info['pruned']:
                    penalty = math.pow(1.5, (info['pruned'] / info['total']) * 10)
                    heapq.heappush(pq, [base_score * penalty / cost, ln, idx, base_score, cost, info['pruned']])
                    continue
        ...
        for name in valid_layer_names:
            m = dict(self.model.named_modules())[name]
            total_c = m.out_channels if hasattr(m, 'out_channels') else m.out_features
            
            # 创建 0/1 掩码
            mask = torch.ones(total_c, device=self.device)
            if name in registry:
                pruned_indices = list(registry[name])
                mask[pruned_indices] = 0.0
            
            # 使用 register_buffer，这样 mask 会随模型保存，且不计入梯度
            if hasattr(m, 'interaction_mask'):
                m.interaction_mask = mask
            else:
                m.register_buffer('interaction_mask', mask)
                
            # 保留索引记录用于统计
            m.keep_indices = sorted(list(set(range(total_c)) - registry[name]))

        return {'actual_sparsity': current_red / self.original_total_params}
    def get_stats(self):
        """
        打印每一层的剪枝统计数据，包括 HSIC 得分和通道保留率
        """
        print("\n" + "="*95)
        print(f"{'Layer Name':<45} | {'IT-Score':<10} | {'Kept/Total':<15} | {'Ratio'}")
        print("-" * 95)
        ...
        for name, m in self.model.named_modules():
            # 检查是否有我们注册的掩码或索引记录
            if hasattr(m, 'interaction_mask'):
                # 从 mask 计算保留的通道数 (mask 中 1.0 的数量)
                kept = int(m.interaction_mask.sum().item())
                total = m.interaction_mask.numel()
                
                total_original_channels += total
                total_kept_channels += kept
                
                # 获取该层在校准阶段计算出的重要性得分
                avg_s = self.layer_avg_scores.get(name, 0.0)
                norm_s = avg_s / (max_s + 1e-8)
                bar = "█" * int(norm_s * 10)
                
                print(f"{name:45} | {avg_s:10.4f} | {kept:>4}/{total:<4} | {kept/total:>6.1%} {bar}")

        print("-" * 95)
        if total_original_channels > 0:
            overall_ratio = total_kept_channels / total_original_channels
            print(f"{'TOTAL CHANNELS':<45} | {'-':<10} | {total_kept_channels:>4}/{total_original_channels:<4} | {overall_ratio:>6.1%}")
        print("="*95 + "\n")
