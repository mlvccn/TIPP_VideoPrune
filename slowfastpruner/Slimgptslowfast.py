...
class SlimGPTPruner:
    def __init__(self, model, sparsity=0.5, device='cuda:0'):
        ...
        self.hessians = {}  # 存储各层的 Hessian 统计量 (X X^T)
        self.nsamples_per_layer = {}
        self.hooks = []
    ...
    def _get_activation_hook(self, name):
        def hook(module, input, output):
            # SlimGPT 关注输入特征 X 的分布
            x = input[0].detach().float()
            
            # 针对 3D 视频特征优化：[B, C, T, H, W] -> [Tokens, C]
            # 结构化剪枝通常针对输出通道，因此 Hessian 基于输入维度 C
            if x.dim() == 5:
                # 调整维度顺序并展平：将 Batch, T, H, W 全部视为 Token 维度
                B, C, T, H, W = x.shape
                x = x.permute(0, 2, 3, 4, 1).reshape(-1, C)
            
            # 计算 Hessian 统计量 (X^T * X)
            current_hessian = x.t() @ x 
            current_n = x.shape[0]
            
            if name not in self.hessians:
                self.hessians[name] = current_hessian.to(self.device)
                self.nsamples_per_layer[name] = current_n
            else:
                self.hessians[name] += current_hessian.to(self.device)
                self.nsamples_per_layer[name] += current_n
        return hook
    ...
    def run_calibration(self, loader, device):
        ...
        self.register_hooks()
        self.model.eval()
        print(">>> 正在采集层间二阶信息特征 (Hessian/SlimGPT)...")
        with torch.no_grad():
    ...
    def prune(self, min_keep_ratio=0.1):
        """
        基于 SlimGPT (OBS) 的结构化贪婪剪枝
        """
        valid_layer_names = [n for n in self.ordered_layer_names if n in self.hessians]
        
        # 1. 准备优先队列
        pq = []
        layer_info = {}
        
        print(f">>> 计算 {len(valid_layer_names)} 层的 OBS 重要性评分...")
        
        for idx, name in enumerate(valid_layer_names):
            m = dict(self.model.named_modules())[name]
            
            # 计算 Hessian 逆矩阵 (SlimGPT 核心优化)
            H = self.hessians[name] / self.nsamples_per_layer[name]
            eps = 1e-6
            H.add_(torch.eye(H.shape[0], device=H.device) * eps)
            
            try:
                # 使用 Cholesky 分解求逆，提高数值稳定性
                L = torch.linalg.cholesky(H)
                H_inv = torch.cholesky_inverse(L)
            except:
                H_inv = torch.inverse(H)
            
            # SlimGPT 结构化评分：对于卷积层，每个输出通道 (Filter) 的得分
            # 公式近似：Score_i = ||W_i||^2 / trace(H_inv) 
            # 这里 W_i 是第 i 个 filter 的权重
            W = m.weight.data.view(m.out_channels, -1) # [Out, In*K*K*K]
            
            # 计算 OBS 扰动得分
            # 简化版：利用 H_inv 的迹或均值来衡量输入端的不确定性对输出 Filter 的影响
            denom = torch.mean(torch.diag(H_inv)) 
            scores = torch.norm(W, p=2, dim=1)**2 / (denom + 1e-8)
            
            # 归一化得分
            scores = (scores - scores.min()) / (scores.max() - scores.min() + 1e-8)
            self.layer_avg_scores[name] = torch.mean(scores).item()
        ...
        with tqdm.tqdm(total=int(target_red), desc="SlimGPT-Pruning") as pbar:
            while current_red < target_red and pq:
                ...
                # 惰性生存机制惩罚：防止某一层被剪空，模拟非均匀分布
                if last_cnt != info['pruned']:
                    # SlimGPT 建议随着剪枝增加，剩余权重的“重要性”非线性上升
    ...
    def get_stats(self):
        """打印每层状态"""
        print("\n" + "="*85)
        print(f"{'Layer Name':<40} | {'Avg Score':<10} | {'Kept/Total':<15} | {'Ratio'}")
        print("-" * 85)
